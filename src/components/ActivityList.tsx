                ...
                state.activities.map( activity => (
                    <div key={activity.id} className="px-5 py-10 bg-white mt-5 flex justify-between shadow">
                        <div className="space-y-2 relative">
                            ...
                            <p className="text-4xl font-black text-lime-500">{activity.calories} <span>calorías</span></p>
                        </div>

                        <div className="flex gap-5 items-center">
                            <button onClick={() => dispatch({type: 'set-activeId', payload: {id: activity.id}})}>
                                <PencilSquareIcon className="h-8 w-8 text-gray-800 hover:text-gray-950"/>
                            </button>

                            <button onClick={() => dispatch({type: 'delete-activeId', payload: {id: activity.id}})}>
                                <XCircleIcon className="h-8 w-8 text-red-500 hover:text-red-800"/>
                            </button>
                        </div>
                    </div>
                ))
            
            : <p className="text-center my-5 text-red-800">No hay actividades enlistadas</p> }
        </>
    )
}
